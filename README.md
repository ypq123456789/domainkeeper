...
- 后台现在支持按当前托管商直接管理 DNS 记录，支持新增、编辑、删除；A / AAAA / CNAME 记录会给出可直接点击的访问链接
- 米表联系方式支持用空格分隔多个内容，并自动识别邮箱、手机号后生成可复制的联系方式卡片
- Contact settings now support built-in Email / WeChat / QQ / Phone fields plus custom key-value entries, and legacy free-text values are migrated automatically.
- Account settings now support DNSHE API Key / Secret and DNSPod Token, and these provider-side domains can be synced into the same domain snapshot.
- DNSHE 子域名同步现在会自动补齐对应一级域名 WHOIS；类型列表示当前唯一的 DNS 托管商，如果仍在 DNSHE 解析就显示 `DNSHE`，如果已经托管到 Cloudflare / DNSPod，就显示对应平台；识别到可直达的控制台地址后，点击类型可直接跳到对应平台 DNS 记录页
- DNSHE 子域名同步现在会自动判断 180 天续期窗口：到窗口内就直接续期，未到窗口则按接口返回的剩余窗口天数反推当前二级域名到期时间
...
- 米表联系方式
- 米表联系方式自动识别与复制展示
- 联系方式现在是结构化配置：默认提供 Email / WeChat / QQ / Phone，也支持继续追加自定义字段，并兼容旧的自由文本联系方式
- 前台是否需要登录
...
- Super admin now shows each customer's frontend entry, public market entry, and connected custom hostname separately
- DNSHE 永不过期子域名现在把 `99991231` / `9999-12-31` 视为同一个“永久”哨兵值，后台不会再把这类域名显示成红色空日期框
...
- Added an in-site `TG Bot` setup guide in simplified Chinese, including BotFather, Token, Chat ID, and test-send steps with screenshot-based walkthroughs.
- The admin `TG 到期提醒` section now includes a `?` help link that jumps directly to `docs#tg-bot-guide`.
- The registrar `官网` control in admin rows is now a clearer button-style dropdown with up/down arrows, and its expanded panel opens below the row instead of covering the registrar input.
- Frontend and admin preview screenshots were re-captured and synced across README, in-site preview cards, and the `DEMO_ASSETS` fallback resources.
